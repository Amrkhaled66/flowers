        ...
        <div className="flex gap-x-3">
          ...
          <div className="flex w-full flex-1 flex-col gap-y-1">
            <div className="flex w-full items-center justify-between text-xs lg:text-sm">
              <p className="font-semibold">{getLocalizedName(products[0])}</p>
              <p className="font-semibold">{priceFormatter(total)}</p>
            </div>
            <p className={`text-xs font-bold text-blue-600 ${shippingStatusTextColor[orderStatus as keyof typeof shippingStatusTextColor]} lg:text-sm`}>
              {orderStatus}
            </p>
            <p className="text-xs lg:text-sm">{DateFormatter(new Date(createdAt), language)}</p>
          </div>
        </div>
      </div>
    </Link>
