...
const parsePriceRange = (minParam: string | null, maxParam: string | null): number[] => {
  ...
  return [isNaN(min) ? INITIAL_FILTER_OPTIONS.price_range[0] : min, 
...
export const useFilterOptions = () => {
  ...
  const updateURLParams = useCallback((newAppliedOptions: FilterOptions) => {
    ...
    if (newAppliedOptions.category_ids.length > 0) {
      newSearchParams.set("category_ids", newAppliedOptions.category_ids.join(','));
    }
    if (newAppliedOptions.occasion_ids.length > 0) {
      newSearchParams.set("occasion_ids", newAppliedOptions.occasion_ids.join(','));
    }
